...
#    HetDfArray = scipy.array(DCs.DataContainers['HetDf'].GetDataArray()).astype(int)
#    HetDfNEzeroArray = (HetDfArray!=0)
#    SNPIDArray = scipy.compress(HetDfNEzeroArray, SNPIDArray)
#    PValArray = scipy.compress(HetDfNEzeroArray, PValArray)

    IndexArray  = []
    XYIndexAray = []
    for i in xrange(len(SNPIDArray)):
        Entry = SNPIDArray[i]
        if(SNPInfoDict.has_key(Entry)):
            IndexArray.append(SNPInfoDict[Entry]['index'])
            XYIndexAray.append(i)
    IndexArray = scipy.array(IndexArray)
    X          = XPosArray[IndexArray]
    ChromArray = ChrArray[IndexArray]

    XYIndexAray = scipy.array(XYIndexAray)
    Y           = -scipy.log10(PValArray[XYIndexAray])
    ...
    if(Arguments.boGreyScale):
        ...
        for Entry in ChromArray:
            ColorArray.append(Colors[int(Entry)-1])
    YInsign = Y < -scipy.log10(1.0e-6/46.0)  # VEffLi = 46
    YSign   = Y > -scipy.log10(5.0e-8/46.0)  # VEffLi = 46
    YSugg   = Y >= -scipy.log10(1.0e-6/46.0) # VEffLi = 46
    YSugg  *= Y <= -scipy.log10(5.0e-8/46.0) # VEffLi = 46
    ...
    YSign = -scipy.log10(scipy.array([5.0e-8/46.0,5.0e-8/46.0])) # VEffLi = 46
    ...
    YSugg = -scipy.log10(scipy.array([1.0e-6/46.0,1.0e-6/46.0])) # VEffLi = 46
...
def main(ExecutableName):
    ...
    XXMax        = None
    XXMin        = None
    XPath               = os.path.join(Arguments.GWAOutputPath,'HapMapB36R24.txt.gz')
    SNPInfoDecompressed = os.path.join(TmpPath,'HapMapB36R24.txt')
#    os.system('ln -sf '+XPath+' '+SNPInfoDecompressed)
    ...
    XXMin = XPosArray.min()
    XXMax = XPosArray.max()
    LogString = '**** Parsed \"HapMapB36R24.txt\" ...'
    print LogString
    Log.Write(LogString+'\n')
    ...
    if(Arguments.GWAOutputFile!=None):
        ...
        for File in os.listdir(Arguments.GWAOutputPath):
            if((re.search(Arguments.GWAOutputExtStr,File)) and
               ...
               (not re.search('.gz',File)) and
               (not re.search('info',File))):
...
#            DCsList.SetPhenotypeName(P)
            ...
            if(Arguments.boGreyScale):
                ...
                RGB_tuples = map(lambda x: colorsys.hsv_to_rgb(*x), HSV_tuples)
            for i in xrange(len(RGB_tuples)):
                List = list(RGB_tuples[i])
                List.append(1.0)
                RGB_tuples[i] = tuple(List)

            DCs = DataContainer.DataContainers()
            DCs.ParseGWAOutput(os.path.join(Arguments.GWAOutputPath,File),
                               Log)
            DCs.Color = []
            for chr in xrange(Arguments.NChr):
                DCs.Color.append(RGB_tuples[chr])
