...
if(False):
    ...
    CompleteGraph = networkx.Graph()
    for NodeId in NodePropertyDict.iterkeys():
        if(NodePropertyDict[NodeId]['NodeInENGAGEMA']=='1'):
            CompleteGraph.add_node(n=NodeId,
                                   attr_dict=NodePropertyDict[NodeId])
    ...
    for Edge in EdgePropertyDict.iterkeys():
        ...
        SourceNodeId = StringList[0]
        TargetNodeId = StringList[1]
        ...
        for s0 in ['GGM','Jaccard','TraitGene']:
            try:
                ...
                StringList.extend(re.sub('Gene:','',re.sub('Trait:','',Edge)).split('*'))
#         for s0 in ['GGM','Jaccard','TraitGene']:
#             if(not EdgePropertyDict[Edge].has_key(s0+'EdgeType')):
#                 EdgePropertyDict[Edge][s0+'EdgeType'] = 'None'
#             if(not EdgePropertyDict[Edge].has_key(s0+'EdgeValue')):
#                 EdgePropertyDict[Edge][s0+'EdgeValue'] = 'None'
        CompleteGraph.add_edge(u=SourceNodeId,
                               v=TargetNodeId,
                               attr_dict=EdgePropertyDict[Edge])

    networkx.write_graphml(G=CompleteGraph,
                           path='Data/CompleteGraph.graphml',
                           encoding='utf-8',
                           prettyprint=True)
    os.system('lbzip2 -f Data/CompleteGraph.graphml')

if(True):
    os.system('lbzip2 -d -k Data/CompleteGraph.graphml.bz2')
    CompleteGraph = networkx.read_graphml(path='Data/CompleteGraph.graphml')
    os.remove('Data/CompleteGraph.graphml')

    NodeAttributes = {}
    for Node in CompleteGraph.nodes():
        try:
            NodeAttributes[Node] = int(CompleteGraph.node[Node]['NodeInENGAGEMA'])
        except:
            NodeAttributes[Node] = -999
    networkx.set_node_attributes(G=CompleteGraph,
                                 name='NodeInENGAGEMA',
                                 attributes=NodeAttributes)
    NodeAttributes = {}
    for Node in CompleteGraph.nodes():
        try:
            NodeAttributes[Node] = int(CompleteGraph.node[Node]['NodeInENGAGE'])
        except:
            NodeAttributes[Node] = -999
    networkx.set_node_attributes(G=CompleteGraph,
                                 name='NodeInENGAGE',
    ...
    for Node in CompleteGraph.nodes():
        try:
            NodeAttributes[Node] = int(CompleteGraph.node[Node]['NodeGroupDeepSplit0'])
        except:
            NodeAttributes[Node] = -999
    networkx.set_node_attributes(G=CompleteGraph,
                                 name='NodeGroupDeepSplit0',
                                 attributes=NodeAttributes)
    NodeAttributes = {}
    for Node in CompleteGraph.nodes():
        try:
            NodeAttributes[Node] = int(CompleteGraph.node[Node]['NodeGroupDeepSplit4'])
        except:
            NodeAttributes[Node] = -999
    networkx.set_node_attributes(G=CompleteGraph,
                                 name='NodeGroupDeepSplit4',
                                 attributes=NodeAttributes)
    EdgeAttributes = {}
    for Edge,Value in networkx.get_edge_attributes(G=CompleteGraph,
                                                   name='GGMEdgeValue').iteritems():
        try:
    ...
    EdgeAttributes = {}
    for Edge,Value in networkx.get_edge_attributes(G=CompleteGraph,
                                                   name='JaccardEdgeValue').iteritems():
        try:
            EdgeAttributes[Edge] = float(Value)
        except:
            EdgeAttributes[Edge] = -999.0
    networkx.set_edge_attributes(G=CompleteGraph,
                                 name='JaccardEdgeValue',
                                 attributes=EdgeAttributes)
    EdgeAttributes = {}
    for Edge,Value in networkx.get_edge_attributes(G=CompleteGraph,
